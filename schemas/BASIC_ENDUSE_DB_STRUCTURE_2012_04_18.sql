...
# Dumping database structure for enduse
CREATE DATABASE IF NOT EXISTS `enduse` /*!40100 DEFAULT CHARACTER SET latin1 */;
USE `enduse`;
...
# Dumping structure for table enduse.data_wl
CREATE TABLE IF NOT EXISTS `data_wl` (
  `record_id` int(10) unsigned NOT NULL AUTO_INCREMENT,
  `site_id` smallint(5) unsigned NOT NULL,
  `source_id` smallint(5) unsigned NOT NULL,
  `file_id` int(10) unsigned NOT NULL,
  ...
  UNIQUE KEY `site_id_source_id_date_time` (`site_id`,`source_id`,`date_time`),
  KEY `site_id_source_id_file_id` (`site_id`,`source_id`,`file_id`),
  CONSTRAINT `data_wl_ibfk_1` FOREIGN KEY (`site_id`, `source_id`, `file_id`) REFERENCES `files` (`site_id`, `source_id`, `file_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=latin1 ROW_FORMAT=COMPACT;

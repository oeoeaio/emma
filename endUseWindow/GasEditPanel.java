...
public class GasEditPanel extends JPanel implements ActionListener{
	...
	public GasEditPanel(Connection dbConn){
		this.dbConn = dbConn;
	...
	private void buildGUI(){
		...
		sourceTypeInput.setSelectedItem("Gas");
		sourceTypeInput.setEnabled(false);
	}

	private void resetGasEditForm(){
		siteIDInput.setText("");
	...
	private void addGas(Site site){
		...
		if (siteIDInput.getText().equals("") && sourceIDInput.getText().equals("")){
			...
			try{
				...
				MySQL_Statement.executeUpdate("INSERT INTO sources (site_id,source_name,source_type,measurement_type) VALUES("+site.siteID+",NULL,'Gas','Pulse')");//creates new row in sources
		...
		if (newSourceID != null){
			...
			int response = JOptionPane.showOptionDialog(null,this,"Adding new gas.",JOptionPane.YES_NO_CANCEL_OPTION,JOptionPane.PLAIN_MESSAGE,null,new String[] {"Done","Add Another","Cancel"},null);
			if (response == JOptionPane.YES_OPTION || response==JOptionPane.NO_OPTION){
				...
				if (newSource.isValid()){
					Gas newGas = new Gas(site,newSourceID,sourceNameInput.getText(),notesInput.getText());
					if (newGas.isValid()){
						try{
							Statement MySQL_Statement = dbConn.createStatement();
							try{
								...
								try{
									String updGasSQL = "INSERT INTO gas (site_id,source_id,notes) VALUES("+site.siteID+","+newSourceID+","+(notesInput.getText().equals("")?"NULL":"'"+notesInput.getText()+"'")+")"; //adds specified information into the database
									...
									sE.printStackTrace();
									JOptionPane.showMessageDialog(null,"Error occured when writing gas information.","Error",JOptionPane.ERROR_MESSAGE);
									addGas(site);
								}
							} catch(SQLException sE){
				...
				removeGas(site,newSourceID); //incomplete source
			}
			resetGasEditForm();
		}
		else{
	...
	private void editGas(Source source){	
		try{
			Statement MySQL_Statement = dbConn.createStatement();
			siteIDInput.setText(source.site.siteID+": "+source.site.siteName);
			sourceIDInput.setText(source.sourceID);
			ResultSet gasData = MySQL_Statement.executeQuery("SELECT sources.source_name,sources.source_type,sources.measurement_type,gas.* FROM gas LEFT JOIN sources ON sources.source_id = gas.source_id WHERE gas.site_id ="+source.site.siteID+" AND gas.source_id = "+source.sourceID); //retrieves data pertaining to selected site
						
			if (gasData.next()){ //if found relevant record
				sourceNameInput.setText(gasData.getString("source_name"));
				sourceTypeInput.setSelectedItem(gasData.getString("source_type"));
				measurementTypeInput.setSelectedItem(gasData.getString("measurement_type"));
				notesInput.setText(gasData.getString("notes"));
	
				int response = JOptionPane.showConfirmDialog(null,this,"Editing gas (Source ID: "+source.sourceID+")",JOptionPane.OK_CANCEL_OPTION,JOptionPane.PLAIN_MESSAGE);
				if (response == JOptionPane.OK_OPTION){
					...
					if (newSource.isValid()){
						Gas newGas = new Gas(source.site,source.sourceID,sourceNameInput.getText(),notesInput.getText());
						if (newGas.isValid()){
							try{
								...
								MySQL_Statement.executeUpdate(updSourceSQL);
								try{
									String updGasSQL = "UPDATE gas SET notes="+(notesInput.getText().equals("")?"NULL":"'"+notesInput.getText()+"'")+" WHERE site_id = "+source.site.siteID+" AND source_id = "+source.sourceID; //adds specified information into the database
									MySQL_Statement.executeUpdate(updGasSQL);
								}catch(SQLException sE){
									JOptionPane.showMessageDialog(null,"Error occured when writing gas information.","Error",JOptionPane.ERROR_MESSAGE);
									editGas(source);
				...
				JOptionPane.showMessageDialog(this,"An error occured when writing data for gas (Source ID: "+source.sourceID+".","Fatal Error",JOptionPane.ERROR_MESSAGE);
			}
		} catch(SQLException sE){
			sE.printStackTrace();
		}
	...
	private void questionRemoveGas(Site site,String sourceID){
		...
		if (response == JOptionPane.YES_OPTION){
			removeGas(site,sourceID);
		}
	}
	
	private void removeGas(Site site,String sourceID){
		try{
			Statement MySQL_Statement = dbConn.createStatement();
	...
	public JPanel getGasButtonPanel(SiteTable siteTable,GasTable gasTable){
		return new GasButtonPanel(siteTable,gasTable);
	}
	
	//Button Panel
	public class GasButtonPanel extends JPanel implements ActionListener{
		private static final long serialVersionUID = -2869230293945872400L;
		
		//Buttons
		JButton gasAddB = new JButton("Add");
		JButton gasEditB = new JButton("Edit");
		JButton gasRemB = new JButton("Remove");

		SiteTable siteTable;
		GasTable gasTable;
		
		GasButtonPanel(SiteTable siteTable,GasTable gasTable){
			this.siteTable = siteTable;
			this.gasTable = gasTable;
			SwingUtilities.invokeLater(new Runnable(){
				public void run(){
					buildGUI();
				}
		...
		private void buildGUI(){
			...
			gasAddB.addActionListener(this);
			gasEditB.addActionListener(this);
			gasRemB.addActionListener(this);
		}
		
		public void setEnabled(boolean enabled){
			gasAddB.setEnabled(enabled);
			gasEditB.setEnabled(enabled);
			gasRemB.setEnabled(enabled);
		...
		public void actionPerformed(ActionEvent aE) {
			if (siteTable.siteListModel.isSelectionEmpty()==false){
				...
				if (aE.getSource().equals(gasAddB)){
					addGas(site);
				}
				else if (aE.getSource().equals(gasEditB)){
					if (gasTable.gasListModel.isSelectionEmpty()==false){
						Source source = gasTable.gasList.get(gasTable.getSelectedRow());
						editGas(source);
					}
					else{
				...
				else if (aE.getSource().equals(gasRemB)){
					if (gasTable.gasListModel.isSelectionEmpty()==false){
						Source source = gasTable.gasList.get(gasTable.getSelectedRow());
						questionRemoveGas(site,source.sourceID);
					}
					else{
						JOptionPane.showMessageDialog(this,"No source selected.","Retrieval Error",JOptionPane.WARNING_MESSAGE);
					}
				}
				gasTable.update(dbConn, site);
			}
		}
	}
}
